...
func (r RabidaImpl) CrawlScrollSmooth(ctx context.Context, job Job, callback func(ret []interface{}, currentPageNo int) bool,
	...
	return r.CrawlScrollSmoothWithConfig(ctx, job, callback, before, after, *r.conf)
}
...
func (r RabidaImpl) CrawlScrollSmoothWithConfig(ctx context.Context, job Job, callback func(ret []interface{}, currentPageNo int) bool, before []chromedp.Action, after []chromedp.Action, conf config.RabiConfig, options ...chromedp.ExecAllocatorOption) error {
	return r.CrawlScrollSmoothWithListeners(ctx, job, func(ctx context.Context, ret []interface{}, currentPageNo int) bool {
		return callback(ret, currentPageNo)
...
func (r RabidaImpl) CrawlScrollSmoothWithListeners(ctx context.Context, job Job, callback func(ctx context.Context, ret []interface{}, currentPageNo int) bool, before []chromedp.Action, after []chromedp.Action, confPtr *config.RabiConfig, options []chromedp.ExecAllocatorOption, listeners ...func(ev interface{})) error {
	...
	if stringutils.IsEmpty(conf.ScrollType) {
		return nil
	}
	var scrollJs string
	switch conf.ScrollType {
	case "scrollTo":
		scrollJs = fmt.Sprintf("window.scrollTo({top: %v, left: %v, behavior: 'smooth'});", conf.ScrollTop, conf.ScrollLeft)
	default:
		scrollJs = fmt.Sprintf("window.scrollBy({top: %v, left: %v, behavior: 'smooth'});", conf.ScrollTop, conf.ScrollLeft)
	}

	ret, err = r.extractScrollSmooth(ctx, job, pageNo, conf)
	if err != nil {
		return errors.Wrap(err, "")
	}

	if abort = callback(ctx, ret, pageNo); abort {
		return nil
	}
	...
	for {
		...
		keepOn, err = func() (bool, error) {
			...
			if err = chromedp.Run(ctx, chromedp.ActionFunc(func(ctx context.Context) error {
				_, exp, err := runtime.Evaluate(scrollJs).Do(ctx)
				if err != nil {
					return err
				}
				if exp != nil {
			...
			DelaySleep(conf, "scroll to retrieve data")
			pageNo++
			logrus.Infof("\n current scrollNo: %v \n", pageNo)
			if r.conf.Debug {
			...
			if ret, err = r.extractScrollSmooth(newCtx, job, pageNo, conf); err != nil {
				goto ERR
			}

			if abort = callback(newCtx, ret, pageNo); abort {
				goto END
			}
...
func (r RabidaImpl) extractScrollSmooth(ctx context.Context, job Job, pageNo int, conf config.RabiConfig) (ret []interface{}, err error) {
	defer func() {
	...
	if stringutils.IsNotEmpty(job.CssSelector.XpathScope) || stringutils.IsNotEmpty(job.CssSelector.Xpath) {
		...
		ret = r.populateXScrollSmooth(ctx, job.CssSelector, conf, doc)
	} else {
		ret = r.populateScrollSmooth(ctx, father, job.CssSelector, conf)
	}
	return
}

func (r RabidaImpl) populateScrollSmooth(ctx context.Context, father *cdp.Node, cssSelector CssSelector, conf config.RabiConfig) (ret []interface{}) {
	...
	for _, node := range nodes {
		if cssSelector.Attrs == nil {
			...
			if stringutils.IsEmpty(cssSelector.Attr) {
				...
				if stringutils.IsEmpty(cssSelector.Css) {
					...
					if cssSelector.Attr == "outerHTML" {
						...
						_ = chromedp.Run(timeoutCtx, chromedp.AttributeValue(cssSelector.Css, cssSelector.Attr, &text, &ok, chromedp.ByQuery, chromedp.FromNode(node)))
						value = text
					}
				}
			}
			...
			for attr, sel := range cssSelector.Attrs {
				result := r.populateScrollSmooth(ctx, node, sel, conf)
				if len(result) > 0 {
...
func (r RabidaImpl) populateXScrollSmooth(ctx context.Context, cssSelector CssSelector, conf config.RabiConfig, html *html.Node) (ret []interface{}) {
